	...
	return (
		<nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-4">
			...
			<div className="sm:hidden w-full">
				...
				{/* Always mounted — open/close driven by CSS transitions */}
				<div
					className={cn(
						"overflow-hidden transition-all duration-300 ease-out",
						"rounded-xl bg-slate-900/90 backdrop-blur-xl border shadow-xl shadow-black/30",
						open
							? "mt-1 max-h-96 opacity-100 translate-y-0 border-slate-600/40"
							: "max-h-0 opacity-0 -translate-y-2 pointer-events-none border-transparent"
					)}
				>
					{/* key on this div remounts the links on every open, replaying the stagger */}
