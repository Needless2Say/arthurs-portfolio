...
#
#  Fresh clone:
#    make setup && make docker-up      # http://localhost:3005/arthurs-portfolio
...
# 3005 keeps this clear of the rest of the ecosystem. 3002 used to be here and
# COLLIDED with kriegerdataforge-auth-ui, whose port is load-bearing: OIDC_ISSUER
# must equal the auth UI's browser origin, and localhost:3002 is baked into every
# app's KDF_OIDC_ISSUER and the return_to guard. This side had to move.
#   3000 fitness-app-frontend  3001 tiffanys-space  3002 auth-ui
#   3003 kriegerdataforge-portfolio  3004 template-nextjs  3005 here
DEV_PORT     ?= 3005
PREVIEW_PORT ?= 4173
BASE_PATH    := /arthurs-portfolio

# ---------- Python (bump-*, the kdf-fmt style gate and the sprite script only) ----------

# This is a TypeScript repo; the venv exists solely for those Python tools.
PYTHON_VERSION ?= 3.14
...
help: ## Show this help message
	$(call banner,arthurs-portfolio - Makefile)
	@awk 'BEGIN { FS = ":.*##" } \
		/^##@/ { printf "\n$(YELLOW)%s$(NC)\n", substr($$0, 5); next } \
...
docker-shell: _ensure-env-local ## Open an interactive shell inside the running portfolio container
	@printf "$(GREEN)Opening a shell in portfolio...$(NC)\n"
	$(COMPOSE) exec portfolio sh
...
docker-clean: _ensure-env-local ## Remove this stack's container, named volumes and local images
	@printf "$(YELLOW)Removing portfolio container, volumes and local images...$(NC)\n"
...
# The PR gate -- green locally before you push. GitHub CI additionally runs secret-scan and
# version-check, which need the PR context and cannot be reproduced here.
...
# PYTHONUTF8=1 is parity with the other repos, whose bump scripts print a U+2705 that crashes
# cp1252 on Windows AFTER writing the files. Harmless where the script is ASCII; it keeps a
# future emoji from turning a successful bump into an apparent failure.
...
codeql-scan-security: ## Run security-extended queries (SARIF)
	...
	@printf "$(GREEN)Results: $(CODEQL_RESULTS)/arthurs-portfolio.sarif$(NC)\n"
...
codeql-scan-quality: ## Run security-and-quality queries (SARIF)
	...
	@printf "$(GREEN)Results: $(CODEQL_RESULTS)/arthurs-portfolio-quality.sarif$(NC)\n"
...
codeql-scan-security-csv: ## Run the security scan (CSV)
	...
	@printf "$(GREEN)Results: $(CODEQL_RESULTS)/arthurs-portfolio.csv$(NC)\n"
...
codeql-scan-quality-csv: ## Run the security-and-quality scan (CSV)
	...
	@printf "$(GREEN)Results: $(CODEQL_RESULTS)/arthurs-portfolio-quality.csv$(NC)\n"
