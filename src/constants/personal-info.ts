...
export const PERSONAL_INFO = {
	...
	bio: "I graduated from the University of Michigan College of Engineering in 2025 with a Bachelor of Science in Computer Science and a minor in Data Science. I currently work as a Software/Platform Engineer at Charles Schwab on the Wealth Asset Management Engineering Team, building dashboards, GitHub devops, and data pipelines. Outside of work I design and build KriegerDataForge, a multi-tenant data platform spanning 18 repositories, with its own OAuth 2.0 and OIDC identity provider, a shared Python SDK that every tenant backend installs, a Terraform control plane, and the products built on top of it.",
	summary:
		"Software and platform engineer at Charles Schwab, on a team building the shared infrastructure other engineers build on. My main focus has been the metadata preservation system, which I designed and built end to end so warehouse governance metadata survives schema deployments, along with the Streamlit dashboards and dashboard template the data governance and tenant teams depend on. Outside work I design and operate KriegerDataForge, a multi-tenant data platform with its own OIDC identity provider, shared SDK, and Terraform control plane. I build systems that are idempotent, observable, and safe to re-run.",
};
...
export const EXPERIENCE: Experience[] = [
	{
		...
		bullets: [
			"Architected and implemented a declarative metadata reconciliation system, mid April to end of August 2026 from the first conversation to working end to end in production, eliminating silent loss of warehouse governance metadata during schema deployments and cutting recovery of thousands of metadata rows from three to four hours of manual rework to seconds, saving an estimated tens to hundreds of hours of manual management",
			"Engineered that system to be provably recoverable, with content hash change detection, fully resumable execution under a hard platform statement timeout, tiered retry with dead-lettering, and eight independent fail closed checks, so any failure mode results in inaction rather than data corruption and recovery is always a safe replay. 20+ schema deployments absorbed since go-live with no human involvement",
			"Designed that system to be object type agnostic and multi-tenant, governing any warehouse object type across both proprietary and fully custom metadata categories with any number of teams managing their own metadata independently, so onboarding a new category is a registry entry with zero engine changes, and built a spreadsheet to catalog generator driven by an issue template so non-engineers author metadata in their preferred tool with no YAML and no engineering dependency",
		...
		resumeBullets: [
			...
			"Engineered it to be provably recoverable, with content hash change detection, fully resumable execution under a hard statement timeout, tiered retry with dead-lettering, and eight independent fail closed checks, so any failure mode is inaction rather than data corruption",
			"Made it object type agnostic and multi-tenant, so a new metadata category is a registry entry with zero engine changes and any team self-serves through a repo template, and built a spreadsheet to catalog generator so non-engineers author metadata with no YAML and no engineering dependency",
