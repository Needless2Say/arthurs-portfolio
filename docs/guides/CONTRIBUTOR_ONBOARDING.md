...
| Tool             | Version                        | Notes                                                       |
| ---------------- | ------------------------------ | ----------------------------------------------------------- |
| **Node.js**      | 24.x                     | Matches the `node:24-alpine` Docker image and CI            |
| **npm**          | bundled with Node 24           | Lockfile is `package-lock.json`. Use `npm`, not yarn/pnpm  |
| **Git**          | any recent                     |                                                             |
...
| **Python**       | 3.14                           | *Optional.* Only for `make bump-*`, the `make ci-style` kdf-fmt gate and the sprite script, `make` creates the `.venv` for you |
...
```bash
make docker-up      # build + start  ->  http://localhost:3005/arthurs-portfolio
make docker-logs    # follow container logs
make docker-stop    # stop (container kept)
make docker-down    # stop AND remove
```

Port **3005**, not 3002. That used to collide with `kriegerdataforge-auth-ui`, whose port cannot
move (`localhost:3002` is baked into every app's `KDF_OIDC_ISSUER` and the OIDC `return_to`
guard). The two can now run side by side.

The container is the whole stack. No backend, no database, and it deliberately does not join
the shared `kdf-net`, because a standalone static site has no sibling service to call.
...
> `make docker-up` (hot reload) or `make serve-static` (the real export).
...
No server only Next features (no API routes, server actions,
...
manually dispatches the "CD" workflow (`cd.yml`) with the version input
