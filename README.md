...
| [`docs/reference/GLOSSARY.md`](docs/reference/GLOSSARY.md) | The glossary. Every term and acronym these docs assume, defined inline |

### How to work here

Read [`AGENTS.md`](AGENTS.md) (this repo's vision + critical rules) → [`WORKFLOW.md`](WORKFLOW.md) (the three lane task loop) → [`skills.md`](skills.md) (the security playbook, before any security sensitive work). Those plus `docs/agent/` are the agentic workflow kit. Centrally synced from `kriegerdataforge-cicd`. Never edit the synced copies locally.
...
- **CSP** is applied via a `<meta http-equiv="Content-Security-Policy">` in
  `src/app/layout.tsx`. It allowlists Google Analytics
...
- **Contact form abuse.** `ContactForm.tsx` has an off screen honeypot
  ...
  `.env.local.example`).
- **Dependencies.** `package.json` pins `postcss` via `overrides` to a
...
Every PR runs five gates. Lint and typecheck, build, npm audit,
...
3. An **approved deployer** manually runs the **"CD"** workflow (`cd.yml`) with the version to deploy.
